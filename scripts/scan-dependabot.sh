...
is_configured_for_dir() {
  ...
  # Check if ecosystem and directory are both mentioned
...
if [[ $MISSING_COUNT -gt 0 ]]; then
  ...
  for result in "${MISSING_RESULTS[@]}"; do
    # Extract ecosystem and directory from result
    eco=$(echo "$result" | sed 's/.*Missing.: \([^ ]*\).*/\1/')
    dir=$(echo "$result" | sed 's/.*directory: \(.*\))/\1/' | tr -d ')')
